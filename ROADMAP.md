...
- [ ] **Free Account Scraper:** Bot to aggregate free VPNs from public sources.
...
## 🛠️ 20 Feature Ideas (Backlog)

### Converter & Subscription
1. **Regex Filtering:** Filter accounts by name pattern.
2. **Emoji Flags:** Add country flags (🇮🇩) to proxy names based on GeoIP.
3. **Expired Cleaner:** Remove accounts with expired dates in names.
4. **Protocol Selector:** `/sub?include=vmess,hy2`.
5. **Custom Proxy Groups:** Allow user-defined group names.
6. **Force SNI/Host:** Overwrite SNI for all accounts (Bug fixing).
7. **UDP Relay Toggle:** Option to enable/disable UDP in config.
8. **Sort by Ping:** Order proxies by latency (cached).

### Checker Tools
9. **Port Scan:** Check common ports (80, 443, 22) in one go.
10. **ICMP Ping:** Real ping (ms) measurement.
11. **Website Check:** Test accessibility of Google/Netflix.
12. **SSL Info:** Show certificate issuer and expiration date.

### Network Utilities
13. **CIDR Calculator:** Subnet masking tool.
14. **MyIP Info:** Show requester's IP and User-Agent.
15. **DNS Checker:** Debug DNS poisoning.
16. **MAC Vendor Lookup:** Identify device manufacturer.
17. **User-Agent Parser:** Analyze browser strings.

### System & Ops
18. **Auto-Update Blocklist:** Daily update for adblock rules.
19. **Cache Warmer:** Script to keep GeoIP cache fresh.
